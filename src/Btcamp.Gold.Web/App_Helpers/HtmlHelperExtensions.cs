...
namespace Btcamp.Gold.Web
{
    public static class HtmlHelperExtensions
    {
        ...
        public static IHtmlString RouteIf(this HtmlHelper helper, string action, string controller, string attribute)
        {
            var currentController =
                (helper.ViewContext.RequestContext.RouteData.Values["controller"] ?? string.Empty).ToString().UnDash();
            var currentAction =
                (helper.ViewContext.RequestContext.RouteData.Values["action"] ?? string.Empty).ToString().UnDash();
            //var curretn = (controller.Equals(currentController, StringComparison.InvariantCultureIgnoreCase) && action.Equals(currentAction, StringComparison.InvariantCultureIgnoreCase) || currentController.Equals(controller, StringComparison.InvariantCultureIgnoreCase));
            var curretn = (controller.Equals(currentController, StringComparison.InvariantCultureIgnoreCase) && action.Equals(currentAction, StringComparison.InvariantCultureIgnoreCase));
            return curretn ? new HtmlString(attribute) : new HtmlString(string.Empty);
        }
